...
watchEffect(async () => {
  store.getUsuario({ id: route.params.id });
  storeDepartamento.getDepartamentos();
  storeCargo.getCargos();
});
...
<template>
  <div class="container">
    <form @submit.prevent="store.updateUsuario({ id: route.params.id })" novalidate>
      <div class="flex flex-col mx-auto md:w-96 w-full">
        <div class="card">
          <div class="card-body">
            <h1 class="text-2xl font-bold mb-4 text-center">Alterar usuário</h1>
            ...
            <div class="flex flex-col gap-2 mb-4">
              ...
              <select v-model="store.form.cargo_id" class="form-input" :disabled="store.loading">
                ...
                <option v-for="cargo in storeCargo.cargos" :key="cargo.id" :value="cargo.id"
                  :selected="store.form.cargo_id == cargo.id">
                  {{ cargo.titulo }}
                </option>
              </select>
            ...
            <div class="flex flex-col gap-2 mb-4">
              ...
              <select v-model="store.form.departamento_id" class="form-input" :disabled="store.loading">
                ...
                <option v-for="departamento in storeDepartamento.departamentos" :key="departamento.id"
                  :value="departamento.id" :selected="store.form.departamento_id == departamento.id">
                  {{ departamento.titulo }}
                </option>
              </select>
